...
public class Priest : RangedTroop
{
    public override IEnumerator Attack()
    {
        ...
        if (closestEnemy != null)
        {
            if(closestEnemy.GetComponent<CharacterStats>().health <= damage) 
            {
                closestEnemy.GetComponent<CharacterStats>().Convert(GetComponent<CharacterStats>().team);       
            }
        }
        CheckForEnemies();
